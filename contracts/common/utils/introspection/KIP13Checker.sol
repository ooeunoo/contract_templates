...
library KIP13Checker {
    ...
    returns (bool)
  {
    ...
    return
      supportsKIP13(account) && _supportsKIP13Interface(account, interfaceId);
  }
    ...
    returns (bool[] memory)
  {
    ...
    if (supportsKIP13(account)) {
      ...
      for (uint256 i = 0; i < interfaceIds.length; i++) {
        interfaceIdsSupported[i] = _supportsKIP13Interface(
          account,
          interfaceIds[i]
        );
      }
   ...
   */
  function _supportsKIP13Interface(address account, bytes4 interfaceId)
    private
    view
    returns (bool)
  {
    bytes memory encodedParams = abi.encodeWithSelector(
      IKIP13.supportsInterface.selector,
      interfaceId
    );
